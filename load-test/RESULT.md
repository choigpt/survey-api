...
**개선: 배치 집계 (60초 주기 스냅샷) 적용 후 재테스트**

| 지표 | Before (실시간) | After (스냅샷) | 변화 |
|------|---|---|---|
| result_query p95 | 995ms | 569ms | **43% 감소** |
| result_query med | 626ms | 34ms | **95% 감소** |
| 처리량 | 31 req/s | 56 req/s | **80% 증가** |
| 총 요청 | 6,453 | 11,483 | 78% 증가 |

스냅샷 hit 시 GROUP BY/COUNT 쿼리 없이 JSON 1건 SELECT로 응답.
중앙값(med) 626ms → 34ms로 대부분의 요청이 극적으로 빨라짐.
...
적용된 최적화: 커넥션 풀 확대 + 페이징 + 인덱스 + 배치 집계 + 벌크 INSERT

### Threshold

| 기준 | 최초 | 최종 |
|------|------|------|
| http_req_duration p95 < 500ms | FAIL | **PASS (286ms)** |
| http_req_failed < 5% | FAIL | **PASS (0%)** |
| submit_response p95 < 1000ms | FAIL | **PASS (391ms)** |
| result_query p95 < 2000ms | FAIL | **PASS (23ms)** |

### 주요 지표

| 지표 | 최초 | 최종 | 개선율 |
|------|------|------|--------|
| http_req_duration p95 | >500ms | **286ms** | 43%+ |
| submit_response p95 | >1000ms | **391ms** | 61%+ |
| result_query p95 | >2000ms | **23ms** | **99% 감소** |
| 처리량 | 39 req/s | **114 req/s** | **2.9배** |
| 에러율 | >5% | **0%** | 완전 해소 |
| 커넥션 타임아웃 | 7,722회 | **0회** | 완전 해소 |
| JVM 메모리 | 1,015MB | **264MB** | **74% 감소** |
| 스레드 peak | 221 | **100** | **55% 감소** |
...
|------|-----|
| hikaricp.connections.timeout | 0 |
| hikaricp.connections.idle | 49 |
| jvm.memory.used | 264MB |
| jvm.threads.live | 100 |
| http.server.requests | 43,974 |

## 고부하 테스트 (850 VU)

읽기 300VU + 쓰기 100VU + 집계 50VU + 동시성 100VU + 스파이크 400VU = 최대 850VU

### Threshold

| 기준 | 결과 |
|------|------|
| http_req_duration p95 < 500ms | **FAIL (591ms)** |
| http_req_failed < 5% | PASS (0.01%) |
| submit_response p95 < 1000ms | PASS (742ms) |
| result_query p95 < 2000ms | PASS (276ms) |

### 주요 지표

| 지표 | 300VU | 850VU | 변화 |
|------|-------|-------|------|
| 처리량 | 114 req/s | **294 req/s** | **2.6배 증가** |
| 총 요청 | 32,479 | **106,556** | 3.3배 |
| http_req p95 | 286ms | 591ms | 스파이크 구간 영향 |
| submit p95 | 391ms | 742ms | 기준 내 |
| result_query p95 | 23ms | 276ms | 기준 내 |
| 에러 | 0 | 14건 (i/o timeout) | 스파이크 400VU 구간 |
| 커넥션 타임아웃 | 0 | 0 | - |
| 메모리 | 264MB | 292MB | 안정 |
| 스레드 peak | 100 | 297 | Tomcat 300 한도 근접 |

### 분석

- **294 req/s**까지 처리 가능. 에러율 0.01%로 거의 안정적
- **p95 초과 원인**: 스파이크 400VU 구간에서 Tomcat 스레드 풀(300) 포화 → i/o timeout 14건
- 커넥션 풀(50)과 메모리(292MB)는 여유 있음
- **병목은 Tomcat 스레드 풀**. max-threads를 500으로 올리면 850VU도 처리 가능할 수 있으나, CPU 바운드 한계에 도달
